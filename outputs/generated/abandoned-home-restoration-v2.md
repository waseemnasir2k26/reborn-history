...
phase_0_classification: transformation-visible (primary) + retention-driver-based (secondary)
include_script: no
target_tool: VEO 3.1
duration_target_minutes: 8-12
language: English (silent / SFX + ambience only, no VO)
scene_duration_seconds: 8
reference_video: https://www.youtube.com/watch?v=GR_jZdrZ6ZA
generated_date: 2026-04-27
status: draft
notes: v2.4.0 enforced 3-part flow. R1 count confirmation, R2+ images→JSON paired per 2 stages. Single-line MJ-ready image prompts.
---

PHASE 0 CLASSIFICATION: transformation-visible (primary) + retention-driver-based (secondary)
...
SECTION COUNT: 35 sections (NO SCRIPT-TO-SCENES PIPELINE — INCLUDE_SCRIPT = no).
...
==================================================
MASTER PROMPT — ABANDONED HOME RESTORATION (v2.0.0)
==================================================
...
Total duration: 91 × 8 sec = 728 sec ≈ 12 min 8 sec (within 8–12 min target — bias toward upper end)

Lean variant for 8-min target: 7 stages × 9 scenes = 63 scenes × 8 sec = 504 sec ≈ 8 min 24 sec.

User may rebalance scenes-per-stage to weight peaks (default 13-per-stage distribution shown):
...
EVERY image prompt MUST be emitted as a full structured block PLUS a single-line tool-ready Midjourney string. BOTH parts mandatory — the structured block is for human review and JSON cross-reference; the single-line string is what the user pastes directly into Midjourney v7.

STRUCTURED BLOCK (13 fields, all required, no blanks):

[IMAGE N — STAGE X — STAGE_LABEL]
Camera: <framing_class> at <focal_length>mm, <camera_height>, <movement>
Lens: <focal_length>mm equivalent
Environment: <continuity-from-previous-scene description; building locked attributes carried forward; era-locked, region-locked, condition state>
Global Condition: completion_pct = <int>%, regression_check = passed
Action: ONE clear action — <verb-object phrase, e.g., "rotten porch board pulled away from joist">
...
Character Lock: image_condition: <BUILDING_REF_URL or interior anchor URL>
Lighting: <stage-derived> — key <direction>, <color_temp_K>K, shadow density <pct>%, source count <int>
Color: <per Color Grading Lock for current stage — exact shadow / highlight / midtone tones>
...
Result: image ready as Scene N start frame + Scene N-1 end frame, stage at <completion_pct>%

SINGLE-LINE MIDJOURNEY-READY STRING (mandatory — appended after the structured block):

`Midjourney prompt: <full descriptive prompt of the frame in 35–60 words, including era, region, building material, decay/restoration state, lighting direction + color temp, foley action, color grade>  --ar 16:9 --v 7 --cref <BUILDING_REF_URL> --cw 100 --style raw`

Example (Stage 4 / Image 27):

`Midjourney prompt: interior of 1920s Pacific Northwest Craftsman bungalow at structural rough-in stage, fresh douglas-fir wall framing visible across full room, copper PEX plumbing rising at corner, electrician's gloved hands stapling romex through stud bay center-frame, mid-afternoon flat daylight pouring through new window openings on screen-right, jobsite clamp lights on tripod, 4200K key, 45 percent shadow density, neutral jobsite color grade, 35mm locked tripod at hip height, 8K ultra detailed, sharp, cinematic, accurate material rendering --ar 16:9 --v 7 --cref https://cdn.example.com/building_ref/craftsman_pnw_t3_v1.png --cw 100 --style raw`

NOTE on `--cw`: use `--cw 100` for full building lock (default for this niche, since the building IS the locked subject). Use `--cw 50` ONLY for interior-anchor scenes where the building exterior is not in frame and only the interior anchor still is being held.

--------------------------------------------------
CHARACTER / SUBJECT CONTINUITY
--------------------------------------------------

LOCK METHOD: veo_image_condition (for VEO 3.1 motion gen) + midjourney_cref (for the Midjourney still-image step)
...
- On every Midjourney still-image regeneration: `--cref <BUILDING_REF_URL> --cw 100`.
...
- For Midjourney still-image regen of interior anchor scenes: `--cref <ROOM_ANCHOR_URL> --cw 100`.
...
TOOL SYNTAX (Midjourney v7 — for the still-image generation step):
- Append to every Midjourney prompt: `--ar 16:9 --v 7 --cref <BUILDING_REF_URL> --cw 100 --style raw`
- For interior anchor scenes (no building exterior visible): `--cref <ROOM_ANCHOR_URL> --cw 100 --style raw`
- Never run a still-image generation without a `--cref` — that is an instant FAILSAFE.
...
EVERY video production has 3 mandatory output parts. The master prompt MUST emit BOTH applicable parts in order. Skipping ANY part means the user cannot produce a video.

PART 1 — SCRIPT (text)
SKIPPED CLEANLY in this build (INCLUDE_SCRIPT = no). No script is ever emitted. No VO length question is ever asked. The SCRIPT-TO-SCENES PIPELINE is NOT present in this master prompt by design.

PART 2 — IMAGE PROMPTS (one per still image)
ALWAYS REQUIRED. Output: N+1 image prompts (92 for the default 91-scene build), each formatted per MASTER IMAGE TEMPLATE — full structured block PLUS the single-line Midjourney-ready string. User feeds these into Midjourney v7 (or Flux 1.1 Pro / Nano Banana with equivalent --cref equivalents) to generate the still images.

PART 3 — SCENE JSON (one per motion clip)
ALWAYS REQUIRED. Output: N scene JSON blocks (91 for the default build), each pairing two adjacent images from Part 2 with motion + audio + camera + lighting metadata. User feeds these into VEO 3.1 (`image_condition` + `audio_prompt_inline`) to generate motion clips.

ENFORCED ORDER (the model MUST emit in this sequence — no reordering, no skipping):

INCLUDE_SCRIPT = no — execution is direct image-prompt + scene-JSON emission. No script. No VO length question. No SCRIPT-TO-SCENES PIPELINE.

RESPONSE 1 — COUNT CONFIRMATION ONLY (no images, no JSON yet):
- Confirm building reference: if REFERENCE IMAGE supplied, that is `BUILDING_REF_URL`. If only TITLE, state that Stage 1 Scene 1 image must be generated first to derive `BUILDING_REF_URL`.
- State the computed counts derived from PACING: e.g., "7 stages × 13 scenes = 91 scenes / 92 images" (or the user-rebalanced distribution).
- State the per-stage allocation: e.g., "Stage 1 = 9 scenes / Images 1–10, Stage 2 = 11 scenes / Images 10–21, Stage 3 = 13 scenes / Images 21–34, Stage 4 = 15 scenes / Images 34–49, Stage 5 = 15 scenes / Images 49–64, Stage 6 = 13 scenes / Images 64–77, Stage 7 = 9 scenes / Images 77–86" — adjust to match actual total.
- Confirm pairing: "2 stages per response, in order: Stages 1+2 → Stages 3+4 → Stages 5+6 → Stage 7 (alone, with closing sequence)."
- STOP. Wait for user to type `continue` before emitting any prompts or JSON.

RESPONSE 2 onward — EMIT 2 STAGES PER RESPONSE. Each response MUST contain ALL of these parts in this exact order:

  a) TRANSITION: Transition image prompt (full MASTER IMAGE TEMPLATE block + single-line MJ string) + Transition JSON for the stage open (1-second black + on-screen STAGE LABEL).

  b) PART 2 — IMAGE PROMPTS for every scene in those 2 stages: emit ONE full MASTER IMAGE TEMPLATE block PLUS the single-line Midjourney-ready string per image. Image numbering is continuous across the whole video (e.g., Stages 1+2 emit Images 1 through 21). Never emit image prompts as bare titles, numbers, descriptions, or "Image N: see scene N".

  c) PART 3 — SCENE JSON for every scene in those 2 stages: emit ONE full JSON block per scene, conforming to the SCENE SYSTEM SCHEMA, all fields filled, valid JSON. Scene numbering is continuous (e.g., Stages 1+2 emit Scenes 1 through 20).

After each response, STOP. Wait for user to type `continue`.

PAIRING DEFAULT:
- Response 2: Stages 1 + 2 (DISCOVERED + ASSESSED) — Transition 1 + Transition 2 + Images for Stages 1–2 + Scene JSON for Stages 1–2
- Response 3: Stages 3 + 4 (DEMOLISHED + STRUCTURED) — same pattern
- Response 4: Stages 5 + 6 (FINISHED + STYLED) — same pattern
- Response 5: Stage 7 ALONE (REVEALED + closing sequence + BEFORE | AFTER split frame + final fade-to-black) — Transition 7 + match-dissolve cue from Stage 6 + Images for Stage 7 + Scene JSON for Stage 7 + closing-scene block

EMIT FORMAT (per image prompt — use this exact shape, both block and single-line string in same emission):
...
Coverage: full frame, foreground action visible
Subjects: hands-only (no faces), max 2 hands per frame OR no humans
Character Lock: image_condition: <BUILDING_REF_URL or interior anchor URL>
Lighting: <key direction>, <K>K, shadow density <pct>%, source count <int>
...
Midjourney prompt: <single-line MJ-ready prompt, 35–60 words> --ar 16:9 --v 7 --cref <BUILDING_REF_URL> --cw 100 --style raw
```

EMIT FORMAT (per scene JSON — use the SCHEMA in SCENE SYSTEM with all values filled).
...
- DO NOT emit image prompts as a list of subject lines, image numbers, or one-line summaries — every image prompt MUST be a fully-filled MASTER IMAGE TEMPLATE block (13 fields) PLUS a single-line Midjourney-ready string with `--ar 16:9 --v 7 --cref <URL> --cw 100`.
- DO NOT skip Part 2 (image prompts) and jump straight to Part 3 (scene JSON) — this is the most common failure mode.
- DO NOT emit all 7 stages at once — the 2-stages-per-response cadence is mandatory.
- DO NOT emit any voiceover or script content — INCLUDE_SCRIPT = no. PART 1 is permanently skipped.
- DO NOT emit JSON before Response 1 has confirmed counts and the user has typed `continue`.
- DO NOT collapse Transition + Images + JSON into a free-form list — keep the (a) Transition → (b) Images → (c) JSON ordering inside every response.

WHY THIS MATTERS: A YouTube video pipeline needs two separate inputs in this build — stills for Midjourney → VEO image_condition, and JSON for VEO motion + audio. Emitting scene JSON without image prompts means the user has no way to generate the stills VEO needs as `image_condition`. The pipeline breaks. Always emit BOTH Part 2 and Part 3 in every prompt-generation response, in that order.
...
FLOW failures (NEW in v2.0.0 — these break the production pipeline at the response level):
- scene JSON emitted without the matching image prompts in the same response (PART 3 without PART 2)
- script content or voiceover line emitted at any point (INCLUDE_SCRIPT = no — PART 1 is permanently skipped)
- all stages emitted in one response (2-stages-per-response cadence violated)
- image prompts emitted as bare titles, numbers, or one-line summaries instead of full MASTER IMAGE TEMPLATE blocks + single-line Midjourney strings
...
- Midjourney still-image generation run without `--cref <URL>` (character lock missing)
